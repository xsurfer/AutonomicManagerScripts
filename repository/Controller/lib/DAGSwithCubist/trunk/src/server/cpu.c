...
void update_cpu_utilization(SERVER_lp_state_type *pointer, time_type now) {
	pointer->cpu_state.total_utilization_oberving_time = 0.99 * pointer->cpu_state.total_utilization_oberving_time
			+ 0.01 * (now - pointer->cpu_state.last_utilization_change_time) * pointer->configuration.max_servents_per_cpu;
	pointer->cpu_state.total_utilization = 0.99 * pointer->cpu_state.total_utilization
			+ 0.01 * (now - pointer->cpu_state.last_utilization_change_time) * (pointer->configuration.max_servents_per_cpu - pointer->cpu_state.available_servents);
	pointer->cpu_state.last_utilization_change_time = now;
...
void cpu_init(SERVER_lp_state_type *pointer) {
	...
	pointer->cpu_state.last_utilization_change_time=0;
	pointer->cpu_state.total_utilization=0;
	pointer->cpu_state.total_utilization_oberving_time=0;
}
