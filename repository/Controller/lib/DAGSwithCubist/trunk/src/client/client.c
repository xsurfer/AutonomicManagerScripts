...
void print_execution_info(state_type *state, time_type now) { //OLD STYLE
	...
	printf("client average statistics: executed transactions: %i (%i%%), committed %i (%i%%), aborted : %i (%i%%), processed events per second: %f\n",
			pointer->committed_transactions + pointer->aborted_transactions,
			(int) (100 * (double) (pointer->committed_transactions + pointer->aborted_transactions) / (double) (pointer->configuration.number_of_transactions)), pointer->committed_transactions,
			(int) (100 * (double) pointer->committed_transactions / (double) (pointer->committed_transactions + pointer->aborted_transactions)), pointer->aborted_transactions,
			(int) (100 * (double) pointer->aborted_transactions / (double) (pointer->committed_transactions + pointer->aborted_transactions)), ProcessedEventsPerSecond());
...
void print_full_final_statistics(state_type *state, time_type now) {
	...
	if (1 || pointer->client_id == 0) {
		int class_id;
		double abort_ratio, response_time, overall_throughput = 0;
	...
	CLIENT_lp_state_type *pointer = &state->type.client_state;
		...
		for (class_id = 0; class_id < pointer->configuration.number_of_tx_classes; class_id++) {
			...
			if (pointer->tx_classes[class_id]->committed_tx_num == 0)
				...
				response_time = (double) pointer->tx_classes[class_id]->tot_response_time / (double) pointer->tx_classes[class_id]->committed_tx_num;
			printf("$;%d;%i;%i;%i;%f;%f;%f\n", class_id, state->num_clients, state->num_servers, state->object_replication_degree, response_time, pointer->tx_classes[class_id]->committed_tx_num / now, abort_ratio);
			/*printf("[%i|%f;%i,%i,%f,%f]\n",
			 class_id,
...
transaction_descriptor *new_transaction(state_type *state, double now) {
	...
	if (pointer->configuration.workload_type == SYNTHETIC) {
		tx = create_new_synthetic_transaction(state, pointer->next_transaction_id++, (double) pointer->configuration.data_items_zipf_const);
		...
		tx = create_new_tpcc_transaction(state, pointer->next_transaction_id++);
		add_transaction(&pointer->transaction_list, tx);
		tx->server_id = get_server(state);
		return tx;
	}
...
void CLIENT_ProcessEvent(unsigned int me, double now, int event_type, event_content_type *event_content, unsigned int size, state_type *state) {
	...
	case INIT:
		...
		pointer->aborted_transactions = 0;
	...
	case DELIVER_MESSAGE:
		...
		case TX_COMMIT_RESPONSE:
			...
			if (pointer->configuration.client_print_execution_info && pointer->client_id == 0 && pointer->ending == 0)
				if ((pointer->committed_transactions + pointer->aborted_transactions) % (int) ((double) pointer->configuration.number_of_transactions * 0.1) == 0)
			...
			if (pointer->committed_transactions + pointer->aborted_transactions < (pointer->configuration.number_of_transactions)) {
