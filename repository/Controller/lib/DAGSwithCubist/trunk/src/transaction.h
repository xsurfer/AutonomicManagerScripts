...
typedef struct _transaction_metadata {
	...
	data_set* read_set;
	/* Data structures for GMU */
	int* read_vector_clock;
	short* has_read_mask;
	int* commit_vector_clock;
	short* nodes_on_write_mask;//the i-th entry is true if this transaction wants to write on the server with vector clock index equals to i. Populated on the prepare.
