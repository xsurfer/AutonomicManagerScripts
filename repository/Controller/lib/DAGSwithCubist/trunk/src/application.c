...
void ProcessEvent(unsigned int me, time_type now, int event_type, event_content_type *event_content, unsigned int size, void *pointer) {
	...
	int i;
	PRINT_EVT_TYPE_TIME printf("event type %d - event time %f\n",event_type,now);
	...
	case CLIENT:
		PRINT_ID_TYPE printf("about to process event (type %d) for client (id = %d)\n",event_type,me);
		CLIENT_ProcessEvent(me, now, event_type, event_content, size, state);

		break;

	case SERVER:
		PRINT_ID_TYPE printf("about to process event (type %d) for server (id = %d)\n",event_type,me);
